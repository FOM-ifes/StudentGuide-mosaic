...
`Die Funktion `cut()` verfügt über eine Option `labels`, mit der Sie aussagekräftigere Namen für die Gruppen angeben können. 
:::
...
:::{.tiefereinsteigen data-latex="{tiefereinsteigen}"}
Es ist ratsam, vom ersten Tag an Regeln für die Auswahl von Variablennamen festzulegen.
:::
...
Es kann oft sinnvoll sein, zusammenfassende Statistiken nach Gruppen zu berechnen und diese in ein Dataframe einzufügen. Die Funktion `group_by()` im Paket `dplyr` erleichtert diesen Prozess. Hier zeigen wir, wie man eine Variable hinzufügt, die das mittlere Alter der Probanden je Substanzgruppe enthält.
