...
Der Students Zweistichproben-t-Test kann ohne (Voreinstellung = default) oder mit der Annahme gleicher Varianzen durchgeführt werden.
...
Die Gruppen können ebenso mit dem Befehl `lm()` (ebenfalls unter der Annahme gleicher Varianz) verglichen werden. Der \textsf{mosaic}-Befehl  `msummary()` gibt eine etwas schlankere Ausgabe im Vergleich zur klassischen Ausgabe des Befehls `summary()` aus.
...
Weitere Details und Beispiele finden Sie in der im Paket `mosaic` enthaltenen Vignette zum Resampling.
:::
...
Die vorangegangenen Vergleiche wurden zwischen zwei Gruppen durchgeführt. Wir können ebenso die Unterschiede zwischen drei oder mehr Gruppen mit einer einfachen Varianzanalys (ANOVA) testen. Hier vergleichen wir den CESD-Score auf Basis des vorwiegenden Drogenkonsums (Heroin, Kokain oder Alkohol), der Median wird hier nun mittels einer Linie und nicht mit einem Punkt dargestellt.
...
Es gibt eine Vielzahl weiterer Vergleichsverfahren, die nach der Anpassung eines ANOVA-Modells verwendet werden können.  Einer davon sind Tukey's Honest Significant Differences (HSD).  Weitere Optionen sind innerhalb des Paketes `multcomp` aufgeführt.
