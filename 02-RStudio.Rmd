...
vom Aussehen und in der Handhabung auf allen ähnlich ist. Die Standardschnittstellen für `R` sind ziemlich unterschiedlich auf den verschiedenen Platformen. Das kann den Studierenden verwirren oder ablenken und führt zu höherem Unterstützungsaufwand für den Dozenten.
- `RStudio` kann in einem Webbrowser laufen.  Zusätzlich zu eigenständigen Desktop-Versionen oder `RStudio.cloud`, kann  `RStudio` als eine Server-Anwendung installiert werden, auf die über das Internet zugreifbar ist. Die Web-Schnittstelle ist fast identisch zu der Desktopversion. Wie bei anderen Web-Services auch, melden Nutzer sich an, um Zugriff zu ihrem Konto zu erlangen. Wenn Studierende sich abmelden und später wieder anmelden, wird ihre Session wiederhergestellt und sie können an der Stelle mit ihrer Analyse weiter machen, wo sie verblieben waren, auch wenn sie sich auf einer anderen Maschine anmelden. Mit einer etwas fortgeschrittenen Einrichtung können Dozenten die Historie der Nutzung von `R` in ihrem Klassenraum speichern und Studierende können diese Historie-Datei in ihre eigene Umgebung laden.
...
::: {.note data-latex=""}
Die Verwendung von `RStudio` in einem Browser ist wie Facebook für Statistik. Jedes Mal, wenn Anwender sich erneut anmelden, wird die vorherige Session wiederhergestellt und sie können weiter machen, wo sie zuletzt aufgehört haben. Nutzer können sich von jedem Gerät mit Internetzugriff anmelden.
:::
...
Obwohl man sicherlich `R` ohne die Verwendung von `RStudio` nutzen kann, vereinfacht `RStudio` viele Dinge und wir empfehlen den Einsatz von `RStudio` stark.  Außerdem erwarten wir in der Zukunft noch weitere nützliche Funktionalitäten, da `RStudio` stetig weiterentwickelt wird.
...
Manchmal kann es nützlich sein zu prüfen, welche Versionen des `mosaic` Pakets, von `R` und
