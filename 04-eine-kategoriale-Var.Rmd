...
## Der Binomial-Test
Ein exaktes Konfidenzintervall für einen Anteilswert (sowie ein Test der Nullhypothese, dass der Bevölkerungsanteil gleich einem bestimmten Wert -- üblicherweise 0.5 -- ist) kann durch die Funktion `binom.test()` berechnet werden. Die Standard-Funktion `binom.test()` benötigt folgendes Eingabeformat: 
```{r, echo=TRUE}
...
Spezifische Informationen werden extrahiert, indem man den Operator `$` oder eine entsprechende Funktion zur Extrahierung nutzt. So kann z. B. das Konfidenzintervall oder der p-Wert ausgegeben werden. 
...
## Der Anteilswert-Test 
In ähnlicher Weise kann ein geschätzest Intervall und ein approximativer Test auf Anteilswerte über die Funktion `prop.test()` ermittelt werden. Die Anzahl der Personen je Kategorie der Variablen `homeless` wird folgendermaßen ausgezählt:
...
Es gibt eine Vielzahl von Gütemaßen, die mit Hilfe bestimmter Referenz-Verteilungen bestimmt werden können. Für die HELP-Daten können wir die Nullhypothese testen, dass in jeder Kategorie des Drogenkonsum ein gleich hoher Anteil an Personen in der Population vorliegt.
...
Alternativ kann  mit Hilfe des Paketes `mosaic` ein analoger `chisq.test()` mit wortreicherer Ausgabe durchgeführt werden.
